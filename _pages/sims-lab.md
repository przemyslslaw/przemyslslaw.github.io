---
layout: archive
title: "The SIMS (Socially Intelligent Media and Systems) Lab"
permalink: /sims-lab/
author_profile: true
---

![image](/images/sims.png){: .align-right width="10%" }
The SIMS Lab studies *how technology shapes society* and how to build *responsible AI-based systems and media*. The SIMS Lab is a **transatlantic** research group at University College Dublin (UCD) and University of Massachusetts Amherst (UMass Amherst).

<style>
.sims-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(220px, 1fr));
  gap: 20px;
  margin: 16px 0 8px;
}
.sims-card {
  border: 1px solid #eee;
  border-radius: 8px;
  padding: 12px;
  text-align: center;
  background: #fff;
}
.sims-photo {
  width: 140px;
  height: 140px;
  object-fit: cover;
  border-radius: 8px;
  background: #f2f2f2;
  display: inline-block;
}
.sims-name {
  margin-top: 10px;
  font-weight: 700;
}
.sims-affil {
  margin-top: 4px;
  font-size: 0.95em;
}
.sims-alumni {
  margin-top: 4px;
  font-size: 0.95em;
  color: #a7a7a7ff;
}
.sims-now {
  margin-top: 4px;
  font-size: 0.95em;
}
</style>

## PhD Students

<div class="sims-grid">
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Soumyadeep_Sar.jpg" alt="Soumyadeep Sar">
    <div class="sims-name"><a href="https://www.linkedin.com/in/soumyadeep-sar-9b1a391b0/">Soumyadeep Sar</a></div>
    <div class="sims-affil">UCD</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Andrew_Yuan.jpg" alt="Andrew Yuan">
    <div class="sims-name"><a href="https://awyuan.github.io">Andrew Yuan</a></div>
    <div class="sims-affil">UMass Amherst</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Chhandak_Bagchi.jpg" alt="Chhandak Bagchi">
    <div class="sims-name"><a href="https://people.umass.edu/cbagchi/">Chhandak Bagchi</a></div>
    <div class="sims-affil">UMass Amherst</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Tabia_Prama.jpg" alt="Tabia Tanzin Prama">
    <div class="sims-name"><a href="https://www.linkedin.com/in/tabia-tanzin-prama-18227018b">Tabia Tanzin Prama</a></div>
    <div class="sims-affil">University of Vermont<br>(informal co-advising)</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Arash_Khojaste.jpg" alt="Arash Khojaste">
    <div class="sims-name"><a href="https://www.linkedin.com/in/arash-khojaste-b21732135/">Arash Khojaste</a></div>
    <div class="sims-affil">UMass Amherst<br>(research assistant)</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Nicholas_Perello.jpg" alt="Nicholas Perello">
    <div class="sims-name"><a href="https://nperello.github.io/">Nicholas Perello</a></div>
    <div class="sims-affil">UMass Amherst</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Xi_Chen.jpg" alt="Xi (Sean) Chen">
    <div class="sims-name"><a href="https://melongone.github.io/">Xi (Sean) Chen</a></div>
    <div class="sims-alumni">UMass Amherst</div>
    <div class="sims-now">Now at Amazon (AGI Foundations)</div>
  </div>
</div>


## MS Students

<div class="sims-grid">
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Gayathri_Sunil.jpg" alt="Gayathri Sunil">
    <div class="sims-name"><a href="https://scholar.google.es/citations?user=Au1gbsgAAAAJ&hl=en&oi=ao">Gayathri Sunil</a></div>
    <div class="sims-affil">UMass Amherst</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Ishita_Chauhan.jpg" alt="Ishita Chauhan">
    <div class="sims-name"><a href="https://ishizz.com/">Ishita Chauhan</a></div>
    <div class="sims-affil">UMass Amherst</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Geetanjali_Aich.jpg" alt="Geetanjali Aich">
    <div class="sims-name"><a href="https://scholar.google.com/citations?user=s9P-0QYAAAAJ&hl=en">Geetanjali Aich</a></div>
    <div class="sims-affil">UMass Amherst</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Durga_Nirmaleswaran.jpg" alt="Durga Nirmaleswaran">
    <div class="sims-name"><a href="https://www.linkedin.com/in/durga-nirmaleswaran/">Durga Nirmaleswaran</a></div>
    <div class="sims-affil">UMass Amherst</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Vishal_Kalakonnavar.jpg" alt="Vishal Kalakonnavar">
    <div class="sims-name"><a href="https://www.linkedin.com/in/vishal-kalakonnavar">Vishal Kalakonnavar</a></div>
    <div class="sims-alumni">UMass Amherst</div>
    <div class="sims-now">Now at WHOOP</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Yashika_Kuckian.jpg" alt="Yashika Jaya Kuckian">
    <div class="sims-name"><a href="https://www.linkedin.com/in/yashika-kuckian-687152214/">Yashika Jaya Kuckian</a></div>
    <div class="sims-alumni">UMass Amherst</div>
    <div class="sims-now">Now at Amazon</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Vijayalakshmi_Vasudevan.jpg" alt="Vijayalakshmi Vasudevan">
    <div class="sims-name"><a href="https://www.linkedin.com/in/vijayalakshmi-vasudevan-b95725156/">Vijayalakshmi Vasudevan</a></div>
    <div class="sims-alumni">UMass Amherst</div>
    <div class="sims-now">Now at Infinitus</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Stephen_Scarano.jpg" alt="Stephen Scarano">
    <div class="sims-name"><a href="https://stephentemp.github.io/">Stephen Scarano</a></div>
    <div class="sims-alumni">UMass Amherst</div>
    <div class="sims-now">Now at Noblis</div>
  </div>
</div>

## Undergraduate Researchers

<div class="sims-grid">
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Aniij_Sharma.jpg" alt="Aniij Sharma">
    <div class="sims-name"><a href="https://www.linkedin.com/in/aniij-s-7690581b2/">Aniij Sharma</a></div>
    <div class="sims-alumni">UCD</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Claire_Murphy.jpg" alt="Claire Murphy">
    <div class="sims-name"><a href="https://www.linkedin.com/in/claire-murphy-107a31282/">Claire Murphy</a></div>
    <div class="sims-alumni">UCD</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Muhammad_Khan.jpg" alt="Muhammad Zubair Khan">
    <div class="sims-name"><a href="https://www.linkedin.com/in/muhammadzubairsamikhan/">Muhammad Zubair Khan</a></div>
    <div class="sims-alumni">UCD</div>
  </div>
  <div class="sims-card">
    <img class="sims-photo" src="/images/sims-lab/Niko_Lorantos.jpg" alt="Niko Lorantos">
    <div class="sims-name"><a href="https://nikolorantos.com">Niko Lorantos</a></div>
    <div class="sims-alumni">UMass Amherst</div>
  </div>
  <div class="sims-card">
    <div class="sims-photo"></div>
    <div class="sims-name">Siyu Zhang</div>
    <div class="sims-affil">UCD</div>
  </div>
  <div class="sims-card">
    <div class="sims-photo"></div>
    <div class="sims-name">Maciej Szmagara</div>
    <div class="sims-affil">UCD</div>
  </div>
  <div class="sims-card">
    <div class="sims-photo"></div>
    <div class="sims-name">Eamonn Martin</div>
    <div class="sims-affil">UCD</div>
  </div>
</div>

## Lab Photos

![SIMS Lab outing at El Comalito](/images/sims-lab/2025_El_Comalito.jpg)
*The US part of our team.*
